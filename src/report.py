...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        markdown = render_markdown_report(evidence=evidence, metrics=metrics)
...
def render_markdown_report(evidence: dict[str, Any], metrics: dict[str, Any]) -> str:
    lines: list[str] = []
    ...
    lines.extend(
        [
            ...
            f"| AI narrative | Not generated in this phase |",
            "",
            "## Executive Summary",
            "",
            ...
            "- This report is deterministic only. It does not score KPIs, assign ratings, or infer undocumented outcomes.",
            "",
        ]
    )

    lines.extend(render_commitment_summary(core))
    lines.extend(render_main_items_delivered(metrics, evidence))
    lines.extend(render_carryover_and_blockers(metrics, evidence))
    lines.extend(render_unplanned_work(metrics, evidence))
    lines.extend(render_kpi_supporting_evidence(metrics))
    lines.extend(render_missing_information(evidence, metrics))
...
def render_main_items_delivered(metrics: dict[str, Any], evidence: dict[str, Any]) -> list[str]:
...
def render_carryover_and_blockers(metrics: dict[str, Any], evidence: dict[str, Any]) -> list[str]:
    carryover = [
        item
...
def render_unplanned_work(metrics: dict[str, Any], evidence: dict[str, Any]) -> list[str]:
...
def render_kpi_supporting_evidence(metrics: dict[str, Any]) -> list[str]:
    timing_rows = metrics.get("timing_evidence", [])
    lines = [
    ...
    for row in timing_rows:
        ...
        first_work = row.get("first_work") or {}
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    escape_md(evidence_label(activation)),
                    escape_md(evidence_label(first_work)),
                    elapsed_label(row.get("elapsed_calendar_hours")),
                    escape_md(row.get("current_group", "")),
                    escape_md("OK" if not warnings else "; ".join(warnings)),
                ]
            )
            + " |"
        )
    ...
    lines.extend(
        [
            ...
            "",
            "Not generated in this deterministic phase. AI-assisted candidate classification will be added later and must not invent impact.",
            "",
            "### Weekly Recap And Monthly Analysis Work",
            "",
            "Not generated in this deterministic phase. AI-assisted extraction will be added later and must only use documented recommendations.",
            "",
        ]
    )
    return lines
...
def render_missing_information(evidence: dict[str, Any], metrics: dict[str, Any]) -> list[str]:
    missing = list(metrics.get("warnings") or []) + list(evidence.get("warnings") or [])
